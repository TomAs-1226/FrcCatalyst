...
|:--|:--|:--|:--|
| Status | Stable: the competition line | Newest code for WPILib alpha-6. Not tagged yet. | Pre-season beta |
...
| `v2.0.0-alpha.2` | `4c81114`, 6 Sep | 2027.0.0-alpha-6 | tag; branch `systemcore` is this plus later docs | fails | The first 2.x built on the WPILib alpha-6 release, which is what image 13 runs. What the beta site documents. |
| 2.0.0-alpha.3 | `5adc688`, 7 Sep | 2027.0.0-alpha-6 | branch `systemcore-alpha6`; no tag | cannot build | alpha.2 plus Autonomy 2.0 and the fix that stops an unsolved MegaTag2 throwing away the MegaTag1 pose beside it. The newest code for image 13. |
...
1. Clone and check out the commit:

   ```bash
   git clone https://github.com/TomAs-1226/FrcCatalyst.git
   cd FrcCatalyst
   git checkout 5adc688
   ```

2. `build.gradle` at `5adc688` still says `version = '2.0.0-alpha.2'`. Change it to
   `'2.0.0-alpha.3'`. Otherwise the build lands in Maven local as a second, different
   2.0.0-alpha.2, which is how that number came to mean two things.
3. Publish it:
...
4. Set the robot project up as in Option 3 of the beta site's
...
| `systemcore` | `v2.0.0-alpha.2` and later docs. Builds the [beta site](https://tomas-1226.github.io/FrcCatalyst/beta/). | 2027.0.0-alpha-6 |
| `systemcore-alpha6` | `v2.0.0-alpha.2` plus twelve commits: Autonomy 2.0, docs and the MegaTag1 fix. The 2.0.0-alpha.3 code. | 2027.0.0-alpha-6 |
