...
| `SimDashboard` (v1.0.0-rc4+) | Generic browser sim cockpit that adapts to any mechanism via `describe()` / `MechanismView`; sim-only, dependency-free |
...
**a self-describing `describe()` / `MechanismView` snapshot driveable in the built-in `SimDashboard` (v1.0.0-rc4+)**.
