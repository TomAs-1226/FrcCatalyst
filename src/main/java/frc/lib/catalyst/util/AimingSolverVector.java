 ...
 * 
 * This class does not interact with motors or network tables, so it is only pure math and code.
...
public class AimingSolverVector {
    ...
    public TargetState calculate(Pose2d robotPose, ChassisSpeeds fieldRelativeSpeeds) {
        ...
        // FIX: Reverted fromRotations to fromDegrees assuming the interpolation map uses degrees.
        ...
        // Clamp against the limits configured in the Builder.
        // Because finalHoodPitch takes the exact limit value, the resulting clampedTheta
        // and finalSpeedMps scale dynamically based on what you set the limits to.
    ...
    public static class Builder {
        ...
        public Builder StaticHood(Rotation2d staticHoodPitch){
            ...
            // FIX: The map now returns the actual static pitch for all distances
            // so the initial 3D vector calculation pulls the correct baseline angle.
